...
export default function TodoList (): JSX.Element {
  ...
  return (
    ...
    {filterCM === 1 && filterUI === 0 && state.todos.filter((todo) => todo.completed).map((todo) =>
      <TodoCard todo={todo} key={todo.id}/>
    )}
    {filterCM === 2 && filterUI === 0 && state.todos.filter((todo) => !todo.completed).map((todo) =>
      <TodoCard todo={todo} key={todo.id}/>
    )}
    {filterCM === 0 && filterUI !== 0 && state.todos.filter((todo) => todo.userId === filterUI).map((todo) =>
      <TodoCard todo={todo} key={todo.id}/>
    )}
    {filterCM === 1 && filterUI !== 0 && state.todos.filter((todo) => todo.completed && todo.userId === filterUI).map((todo) =>
      <TodoCard todo={todo} key={todo.id}/>
    )}
    {filterCM === 2 && filterUI !== 0 && state.todos.filter((todo) => !todo.completed && todo.userId === filterUI).map((todo) =>
      <TodoCard todo={todo} key={todo.id}/>
    )}
  </div>
  )
